...
describe('basic', function () {
  ...
  it.skip('vimeo.com should return open graph data', function () {
    return ogs({
      ...
      expect(result.ogLocale).to.be.eql('en');
      expect(mediaResult(result.ogImage).url).to.include('vimeocdn.com');
      expect(mediaResult(result.ogImage).width).to.be.eql('1280');
      expect(mediaResult(result.ogImage).height).to.be.eql('720');
      expect(mediaResult(result.ogVideo).url).to.include('player.vimeo.com/video/232889838');
      expect(mediaResult(result.ogVideo).width).to.be.eql('1280');
      expect(mediaResult(result.ogVideo).height).to.be.eql('720');
      expect(mediaResult(result.ogVideo).type).to.be.eql('text/html');
      expect(mediaResult(result.twitterImage).url).to.include('vimeocdn.com');
      expect(mediaResult(result.twitterPlayer).url).to.include('player.vimeo.com/video/232889838');
      expect(mediaResult(result.twitterPlayer).width).to.be.eql('1280');
      expect(mediaResult(result.twitterPlayer).height).to.be.eql('720');
