...
describe('image', function () {
  ...
  it('Test getting the description and images from meta tags', function () {
    return ogs({
      ...
      const ogImages = mediaResults(result.ogImage);
      expect(ogImages.length).to.be.eql(45);
      let imageFound = false;
      for (let i = 0; i < ogImages.length; i += 1) {
        if (ogImages[i].url === 'https://web.archive.org/web/20170608000236im_/https://pbs.twimg.com/media/DBwS1I_W0AACC8s.jpg') {
          imageFound = true;
        }
      }
